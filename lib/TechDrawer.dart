...
/// returns a Drawer suited for the app, with the user information from userRep, and highlighting the current page being used.
SafeArea techDrawer2(UserRepository userRep, BuildContext context,
    DrawerSections currentSection) {
  final GlobalKey<ScaffoldState> _key = GlobalKey<ScaffoldState>();
  return SafeArea(child: ClipRRect(
      borderRadius: BorderRadius.only(topRight: Radius.circular(20.0),bottomRight:Radius.circular(20.0)),child: Container(width: MediaQuery.of(context).size.width*0.7,
     ...
     ,currentAccountPicture: CircleAvatar(backgroundColor: secondColor,))*/
              ...
              drawerListTile2("Home",Icons.home_rounded,DrawerSections.home,currentSection, context, userRep,_key),
              drawerListTile2("Profile",Icons.person,DrawerSections.profile,currentSection, context, userRep,_key),
              drawerListTile2("Notifications",Icons.notifications,DrawerSections.notifications,currentSection, context, userRep,_key),
              drawerListTile2("Favorites",Icons.favorite,DrawerSections.favorites,currentSection, context, userRep,_key),
              drawerListTile2("Chats",Icons.chat,DrawerSections.chats,currentSection, context, userRep,_key),
              Spacer(),
              AboutListTile(
                icon: Icon(
                  Icons.info,
...
/// creates a listTile for the drawer, with the relevant pageName,icon,tileSection for the tile.
/// and the currentSection of the drawer.
ListTile drawerListTile2(String pageName,IconData icon,DrawerSections tileSection,DrawerSections currentSection, BuildContext context, UserRepository userRep,GlobalKey<ScaffoldState> key) {
  return ListTile(
    selected: currentSection == tileSection,
    leading: Icon(
      icon,
